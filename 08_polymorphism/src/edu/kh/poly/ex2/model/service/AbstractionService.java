...
import edu.kh.poly.ex2.model.vo.KJHCalculator;
...
//		Animal a = new Animal(); //Cannot instantiate the type Animal > 안된다.
		
		// 해결 방안
		// Animal을 상속 받은 자식 클래스를 이용해 객체를 생성해야 한다.
...
//		KH k1 = new KH();
	...
	System.out.println("-----------------------------------");
	for(int i = 0; i<arr.length ; i++) {
		arr[i].lesson();   // 그냥 KH가 갖고잇는 lesson (단순하게 생각 , 정적)
		// 정적바인딩 :  KH.lesson() (추상 메서드)
		// 동적 바인딩 :
		// arr[0] -> HSH.lesson();
		// arr[1] -> HSH.lesson();
	}
