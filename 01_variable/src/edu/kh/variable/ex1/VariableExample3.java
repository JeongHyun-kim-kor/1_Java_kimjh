...
//		int Num1; // Duplicate local variable Num1
		...
		int abcdefgh;
		int showMeTheMoney;
		int sfdskdflksjflksjsdkjfasdadasd; // 길이 제한 X
...
//		int 1age; // Syntax error on token "1", delete this token 
		...
		// (권장) 5. 여러 단어 이름은 단어의 첫 글자는 대문자로 한다.
		//	단, 첫 시작 글자는 소문자
		// == 카멜 표기법
		// 오류는 발생하지 않음(개발자들 간의 약속/관례)
		
		int userAge; // user + age / 카멜 표기법
		int helloworld;
		int HelloWorld;
		int Helloworld;
		...
		System.out.println(name + age + 10 + 20);                                     
		// 김정현님은 28세 남성입니다.
		...
		// 상수(무조건 대문자로 표기, 여러 단어면 _로 구분)
		final int CONSTANT_NUMBER = 100000000; // 1억
		// 마지막으로 값을 대입하는 변수 = 다시는 값을 대입하지 않겠다.
		// = 변하지 않는 수(상수)
