...
public class WhileExample {
	...
	public void ex1() {
		...
		public void ex2() {
			...
			int sum = 0;
			int input = -1 ; 
			// -1 : 오류가 없다는의미 //while문 내에서 입력 받은 값을 저장할 변수	
			
			// 입력 받은 값이 0인 경우 라는 조건을 판별하기 위해			
			//while문보다 앞에 먼저 선언
			
			// 하지만 input 변수 선언시 초기화 되어있지 않으면 
			// 조건식 수행을 못하기 때문에 
			...
			while(input != 0) { // 에러 -> ==input 변수가 해석되지 않는다 == input변수가 존재하지 않는다.
	...
	public void ex3() {
		...
		// 1) 메뉴판 > while  2) 9번 나올때까지 반복 3) 문자열 누적(고른메뉴들) 숫자누적(가격)
		
		
		Scanner sc = new Scanner(System.in);
		
		//int input = -1; // 아까 while문에 잇는 조건식은 input != 0 이었으니까
		int input = 0;//지금은 input = 0 가능
		
		String menu = ""; // String의 ""리터럴 표기법
		// "" == 빈 문자열 (내용은 없지만 문자열)
...
//		menu += "김밥";
		...
		boolean flag = false; // 신호로 사용하는 변수 / 메뉴 뒤 ,찍기?
		// true : 첫 번째 아닌 주문
		// false : 첫 번째 주문
