...
public class ArrayExample {
	...
	public void ex1() {
		...
		// int형 변수 4개를 묶음으로 다룰 수 잇는 배열을 생성하고 
		//이를 arr이라고 부름
...
//		System.out.println(a);
		...
		arr[0] = 100 ; // arr int[] 참조형 변수가 참조하고 있는 
  					   // 배열의 0번 인덱스에 100을 대입한다.
		
		arr[1] = 200;  // int배열 참조형 변수 arr이 참조하고 있는 배열의 1번 인덱스에 200대입?
		arr[2] = 300;  // 
		arr[3] = 400;  // 
		
		// ** for문을 이용한 배열에 저장된 모든 갑 출력 **
		...
		// int 10개를 저장하는 배열의 각 인덱스(요소)에 
		// 10,20,30 ~ 100까지 대입
		
		int[] arr2 = new int[10]; // index 0 ~ 9번
...
//			System.out.println(arr2[i]);
	...
	public void ex3() {
		...
		//더블형 배열을 참조하는 heightarr을 생성하고 그 시작 주소를 heightarr에 
		// 3교시 11시 45분 // 듣기
...
//		System.out.println(heightArr); 
		// [D@6442b0a6  배열의 (시작)주소가 출력됨 
		//-> 왜?  heightArr 참조 변수에는 
		//배열의 시작주소가 저장되어 있기 때문에
		
		// Arrays.toString(배열명) : 배열에 저장된 모든 값을 한줄로 간단히 출력
//		System.out.println(Arrays.toString(heightArr));
	...
	public void ex6() {
		...
		String[] menuArr = new String[]  {"김밥+라면", "맘스터치", "서브웨이", "KFC", "백반", "순대국", 
										"빵","핫도그","백반","파스타","삼겹살","순대국"};
		
		// 배열 index 범위 내의 난수 발생
	...
	public void ex7() {
		...
		for(int i = 0 ; i < scoreArr.length; i++) {
			...
			sum += scoreArr[i]; // 점수를 입력 받자마자 sum에 입력받은 점수 누적
		}
		...
		public void ex8() {
			...
			for(int i = 0; i < arr.length; i++) {
				...
				if(arr[i] == search) { // arr[i]의 값이 입력받은 값과 같다면
...
//				
		...
		public void createLottoNumber() {
			...
			for(int i=0; i < lotto.length; i++) {  // 화살표를 오른쪽으로 이동시키는 코드
				
				int ran = (int)(Math.random() *45 + 1); // 1~45까지의 난수
				
				
				lotto[i] = ran; // 난수를 배열 요소에 대입
				
				// 난수 중 중복이 발생하면 지우고 다시 입력하는 것보다
				// 그 전 인덱스로 이동후 다시 원래 인덱스로 돌아옴? 제자리로 돌아올 때 난수를 발생시켜 덮어쓰기
				
				// 현재 인덱스(i) 이전의 요소를 순차 접근
				for(int x=0 ; i < x ; x++) {
					
					if(lotto[x] == ran) {  // ran == lotto[i]
						i--; // i 값을 인위적으로 감소시켜 
							// 바깥쪽 for문의 증감식(i++)이 실행되었을 때
		...
		public void ex9() {
			...
			// 복사본의 값을 변경 --> 원본도 값이 변함 == 값을 공유(얕은 복사의 특징)
			copyArr[2] = 10000;
		...
		public void ex10() {
			...
			// 1) for문을 이용한 방법(index가 arr과 copyArr가 서로 동일하다는 특징 이용)
...
//			System.arraycopy(arr, 0, copyArr, arr.length);
			
//			3. 복사할 매열 참조 변수 = Arrays.copyOf(원본배열, 복사할 길이)
			copyArr = Arrays.copyOf(arr, 0);
		...
		public void ex11() {
			...
			//결과: false --> arr1의 참조변수가 무언가를 참조하고 있다.를 의미
...
//			int[] arr2; 
			...
			int[] arr2 = null; // null도 하나의 값
			//배열 참조 변ㄴ수 선언 및 null 초기화 == 값은 있으나, 참조하는게 없다라는 뜻 
			// null 은 비어있는 것이 아님
